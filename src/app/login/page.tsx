            ...
            <>
              ...
              <div className="space-y-2.5">
                ...
                <OAuthButton
                  ...
                  onClick={() => handleOAuth("facebook", signInWithFacebook)}
                  icon={<FacebookIcon />}
                  variant="facebook"
                />
              </div>
              ...
              <p className="mt-4 text-center text-sm text-foreground/60 font-medium">
                {mode === "login" ? (
                  <>New to Sariro?{" "}
                    <button type="button" onClick={() => { setMode("signup"); setError(null); }} className="text-brand-blue font-semibold hover:underline cursor-pointer">Create an account</button>
                  </>
                ) : (
                  <>Already have an account?{" "}
                    <button type="button" onClick={() => { setMode("login"); setError(null); }} className="text-brand-blue font-semibold hover:underline cursor-pointer">Sign in</button>
                  </>
                )}
              </p>
            </>
...
function OAuthButton({
  label, loading, onClick, icon, variant = "light",
}: {
  label: string; loading: boolean; onClick: () => void;
  icon: React.ReactNode; variant?: "light" | "dark" | "facebook";
}) {
