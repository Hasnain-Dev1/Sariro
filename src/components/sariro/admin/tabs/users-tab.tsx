  ...
  async function changeRole(user: Profile, action: RoleAction) {
    try {
      ...
      if (error) {
        ...
        // If the error is about the "role" column not existing, give a helpful message
        if (error.message.includes("column") && error.message.includes("role")) {
          throw new Error("The 'role' column doesn't exist in your profiles table. Run the latest schema.sql in your Supabase SQL Editor to add it.");
        }
        throw error;
      }
